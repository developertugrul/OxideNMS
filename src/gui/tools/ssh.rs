...
impl ToolScreen for SshTool {
    ...
    fn draw(&mut self, ui: &mut egui::Ui, dil: Language) -> Option<ToolEvent> {
        ...
        ui.horizontal(|ui| {
            if ui.add_enabled(!is_connecting, egui::Button::new(t(dil, Message::SshConnect))).clicked() {
        ...
        match &self.state {
            ...
            SshState::Connecting => {}
            SshState::Error(e) => {
                ui.colored_label(egui::Color32::from_rgb(255, 80, 80), format!("{} {}", t(dil, Message::ErrorPrefix), e));
            }
            SshState::Done(config) => {
                let mut return_event = None;
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new(t(dil, Message::SshSuccess)).color(egui::Color32::from_rgb(80, 255, 80)));
                    
                    if ui.button(t(dil, Message::DiffSaveToDb)).clicked() {
                        match db::get_connection() {
                            Ok(conn) => {
                                let device_id = match db::devices::get_or_create_device(&conn, &self.ip, &self.ip) {
                                    Ok(id) => id,
                                    Err(_) => 1, // Fallback
                                };
                                ...
                                if let Ok(history) = db::devices::get_config_history(&conn, device_id) {
...
fn connect_and_fetch(ip: &str, port: &str, user: &str, pass: &str) -> Result<String, String> {
    ...
    let exec = session.open_exec().map_err(|e| format!("Exec kanalı açılamadı: {:?}", e))?;
    let vec: Vec<u8> = exec.send_command("show running-config").map_err(|e| format!("Komut gönderilemedi: {:?}", e))?;
