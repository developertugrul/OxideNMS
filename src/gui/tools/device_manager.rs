...
impl DeviceManagerTool {
    ...
    fn fetch_devices(&self) -> Vec<(i32, String, String, String)> {
        ...
        if let Ok(conn) = db::get_connection() {
            if let Ok(mut stmt) = conn.prepare("SELECT id, name, ip_address, username FROM devices ORDER BY id DESC") {
    ...
    fn delete_device(&mut self, id: i32) {
        if let Ok(conn) = db::get_connection() {
            ...
            if conn.execute("DELETE FROM devices WHERE id = ?1", [id]).is_ok() {
...
impl ToolScreen for DeviceManagerTool {
    ...
    fn draw(&mut self, ui: &mut egui::Ui, dil: Language) -> Option<ToolEvent> {
        ...
        if !self.status_msg.is_empty() {
            ui.add_space(8.0);
            ui.label(egui::RichText::new(&self.status_msg).color(egui::Color32::from_rgb(100, 200, 100)));
        }
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            egui::Grid::new("device_grid").striped(true).spacing([20.0, 8.0]).show(ui, |ui| {
